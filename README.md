...
Использую гибридный пайплайн CV + UNet + OCR, потому что:

- OpenCV дает быстрый и понятный бейзлайн по стенам без обучения моделей.
- UNet с ResNet34 на 4 класса (фон, стена, дверь, окно) лучше держит сложные планы и позволяет добавлять новые классы.
- segmentation-models-pytorch упрощает эксперименты с энкодерами, лоссами и аугментациями.
- Отдельный скрипт `pipeline_cv.py` собирает CV, UNet и OCR в один JSON формат, что удобно для дальнейшего 2D/3D пайплайна.
- `eval.py` честно сравнивает OpenCV и UNet на одних и тех же данных через IoU, tolerant IoU и среднюю дистанцию в пикселях.

---

## Слабые места текущего прототипа

- Чувствительность к "грязным" фото: перспектива, тени, низкий контраст пока обрабатываются слабо, нет отдельного блока нормализации.
- Линейное представление: и GT, и предсказания приводятся к линиям с фиксированной толщиной, из-за этого метрики немного занижены относительно классической dense сегментации.
- Простая векторизация: возможны небольшие зазоры между сегментами, неидеальные углы, лишние короткие отрезки, нет полноценного поиска контуров помещений.
- OCR базовый: EasyOCR используется "из коробки", без дообучения и без отдельной логики для размеров и названий комнат.
- Прототипный формат: сейчас это исследовательский CLI пайплайн, без отдельного API слоя и продакшн обвязки.
